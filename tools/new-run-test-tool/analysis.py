...
def process_wtg(
    ...
    def check_window(s, e, is_base_window=True, allow_disqualified=False):
        ...
        return {
            ...
            'nominal_hours': round(nominal_hours, 2),
            'energy_mwh': round(energy_kwh / 1000.0, 2) if np.isfinite(energy_kwh) else 0.0,
            'energy_kwh': round(energy_kwh, 2) if np.isfinite(energy_kwh) else 0.0,
            'total_alarms': total_alarms,
            'alarm_count': alarm_count_val,
            'warning_count': warning_count_val,
            'fault_codes': fault_counts,
            'three_strike_pass': three_strike_pass,
            'violating_codes': violating_codes,
            'viable': viable,
            'issues': issues,
            'disqualified_bins': disallowed_count
        }
